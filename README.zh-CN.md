...
公开发布前请按 [docs/RELEASE_CHECKLIST.md](docs/RELEASE_CHECKLIST.md) 检查。
...
上传到 GitHub 前，请确认 `LICENSE`、`AI_DISCLOSURE.md`、`.env.example`、`config.example.yaml`、CI workflow 和发布文档都存在；不要把 `.env`、`config.yaml`、本地 runtime status 快照、日志、数据库、缓存或构建 zip 当作源码上传。请先推送到 private GitHub repository，确认 GitHub Actions 通过并验证 Windows artifact，再决定公开仓库或发布 `v0.9.0-rc1` 之类的首个候选版本。
